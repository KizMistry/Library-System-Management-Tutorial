...
-- We will need to join tables - issued_status == members == books == return_status
-- Then filter out books which have been returned
-- Then check if overdue is over 30 days
...
-- Store Procedures 
/*
This will update the Status in the Books table when a entry is made
in the return_status table
In the parameters for the procedure, we will add the names of info we will enter e.g p_return_id etc
we used 'p_' to identify it as a parameter in the code
*/
...
DECLARE
	-- Any variables used in this procedure need to be declared here
	v_isbn VARCHAR(50);
...
BEGIN
	...
	--Finding the isbn and book title and store them in variables
	SELECT
		issued_book_isbn,
		issued_book_name
		INTO
	...
	--Message to notify code has been run
...
SELECT * FROM books;
SELECT * FROM return_status;
-- we need info from each of these tables so we need to join them
